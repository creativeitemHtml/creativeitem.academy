...
@elseif ($lesson_details->lesson_type == 'video-url')
    <div class="overflow-hidden bd-r-10 mb-16 h-314 h-md-428 position-relative bg-light">
        <div class="plyr__video-embed " id="player">
            <iframe width="560" height="315" src="{{ $lesson_details->lesson_src }}?https://plyr.io&amp;iv_load_policy=3&amp;modestbranding=1&amp;playsinline=1&amp;showinfo=0&amp;rel=0&amp;enablejsapi=1" title="YouTube video player" frameborder="0"
...
@elseif($lesson_details->lesson_type == 'google_drive')
    ...
    <div class="overflow-hidden bd-r-10 mb-16 h-314 h-md-428 position-relative bg-light">
        <video id="player" playsinline controls>
            <source class="remove_video_src" src="https://www.googleapis.com/drive/v3/files/{{ $video_id }}?alt=media&key={{ get_settings('youtube_api_key') }}" type="video/mp4">
        </video>
    </div>
    @include('frontend.course_player.player_config')
...
@elseif($lesson_details->lesson_type == 'document_type')
    <div class="overflow-hidden bd-r-10 mb-16 h-314 h-md-428 position-relative bg-light">
        @if ($lesson_details->attachment_type == 'pdf')
            <iframe class="embed-responsive-item" width="100%" height="500px" src="{{ asset('uploads/lesson_file/attachment/' . $lesson_details->attachment) }}" allowfullscreen></iframe>
        @elseif($lesson_details->attachment_type == 'doc')
            <iframe src="https://view.officeapps.live.com/op/embed.aspx?src={{ asset('uploads/lesson_file/attachment/' . $lesson_details->attachment) }}" width='100%' height='650px' frameborder='0'></iframe>
        @elseif($lesson_details->attachment_type == 'txt')
            <iframe src="{{ asset('uploads/lesson_file/attachment/' . $lesson_details->attachment) }}" width='100%' height='650px' frameborder='0'></iframe>
        @endif
    </div>
@else
    <div class="overflow-hidden bd-r-10 mb-16 h-314 h-md-428 position-relative bg-light">
        <iframe class="embed-responsive-item" width="100%" height="550px" src="{{ $lesson_details->lesson_src }}" allowfullscreen></iframe>
    </div>
@endif
