            ...
            @foreach ($sections as $section)
                ...
                <div class="accordion-item">
                    <h2 class="accordion-header">
                        <button class="accordion-button @if ($active_section != $section->id) collapsed @endif" type="button" data-bs-toggle="collapse" data-bs-target="#collapse_{{ $section->id }}" aria-expanded="@if ($section->id != $active_section) false @else true @endif"
                    ...
                    <ul>
                        @foreach ($lessons as $key => $lesson)
                            <li class="tutorial-list-single">
                                <a href="{{ route('course.player', ['slug' => $course_details->slug, 'id' => $lesson->id]) }}" class="tutorial-list-item">
                                    <div class="d-flex align-items-center gap-2">
                                        <input class="form-check-input checkbox-radio-check-input" type="checkbox" value="" id="">
                                        <div class="play-lock-number">
                                            <span>
                                                @if (in_array($lesson->lesson_type, ['text', 'document_type', 'iframe']))
                                                    <i class="fa-solid fa-file"></i>
                                                @elseif (in_array($lesson->lesson_type, ['video-url', 'system-video', 'vimeo-url']))
                                                    <i class="fa-solid fa-video"></i>
                                                @elseif ($lesson->lesson_type == 'image')
                                                    <i class="fa-solid fa-image"></i>
                                                @elseif ($lesson->lesson_type == 'google_drive')
                                                    <i class="fa-brands fa-google-drive"></i>
                                                @else
                                                    <i class="fa-solid fa-file"></i>
                                                @endif
